...
export class ConnectedShipping extends ConnectedShippingRead {
  override forReturn():ConnectedShipping {return new ConnectedShipping(this.commerce,this.rootSecret,this.execute,true);}
  private async claim(sql:PoolClient,e:Exchange,offer:Offer,op:Operation,kind:'label'|'label_refund'|'return_label'|'return_label_refund',extra:Record<string,unknown>={}) {
    if(e.mode!=='live' || op.mode!==e.mode || op.exchangeId!==e.id || op.version!==this.version(e,offer) || op.kind!==kind) {
    ...
    const saved=(await sql.query(`select * from pilot_operations where id=$1 and exchange_id=$2 and mode=$3
      and kind=$4 and version=$5 for update`,[op.id,e.id,e.mode,kind,this.version(e,offer)])).rows[0];
  ...
  async purchase(e:Exchange,offer:Offer,op:Operation,verifyPayment:()=>Promise<void>) {
    ...
    const {receipt,shipping}=await this.call(e,offer,'CreateTransaction',{
      rate:quote.rateId,label_file_type:'PDF',metadata:shippoOperationMetadata(op.id),async:false,
    },'spend',async(sql,current)=>{
      if(this.returning) {
        const plan=current.returnPlan;
        if(!plan || plan.shipping!=='label_pending' || plan.version!==op.version || plan.funding!=='seller_absorbed' || plan.cancelApprovedBy?.length
          || ![current.buyerId,current.sellerId].every(id=>plan.approvals.includes(id))
          || Object.keys(current.operationIssues ?? {}).some(id=>id!==op.id)) conflict('Both exact return approvals and resolved operations are required.');
        await this.claim(sql,current,offer,op,'return_label');
        return;
      }
    ...
    const binding:ShippoTransactionBinding={operationId:op.id,accountOwner:shipping.accountOwner,mode:'live',rateId:quote.rateId,
      parcelId:shipping.parcelId,artifact:quote.artifact,qrRequested:shipping.qrRequested,carrierToken:shipping.carrierToken};
  ...
  async requestRefund(e:Exchange,offer:Offer,op:Operation,label:Operation) {
    const kind=this.returning?'return_label':'label';
    if(!label.providerId || label.kind!==kind || label.exchangeId!==e.id || label.version!==this.version(e,offer)) conflict('A recorded postage purchase is required.');
    ...
    const {receipt,shipping}=await this.call(e,offer,'CreateRefund',{transaction:transactionId,async:false},'reconcile',async(sql,current)=>{
      if(this.returning) {
        const plan=current.returnPlan;
        if(!plan || ![current.buyerId,current.sellerId].every(id=>plan.cancelApprovedBy?.includes(id)) || plan.shipping!=='label_ready'
          || plan.droppedAt || plan.carrierAcceptedAt) conflict('Only mutually cancelled unused return postage can be refunded.');
      } else if(current.payment!=='refunded' || current.sellerDroppedAt || current.carrierAcceptedAt
      ...
      const currentLabel=(await sql.query('select provider_id from pilot_operations where id=$1 and exchange_id=$2 and kind=$4 and version=$3',
        [label.id,e.id,this.version(e,offer),kind])).rows[0];
      if(currentLabel?.provider_id!==transactionId) conflict('The postage transaction changed.');
      await this.claim(sql,current,offer,op,this.returning?'return_label_refund':'label_refund',{transactionId});
    });
