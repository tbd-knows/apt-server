...
try {
  ...
  try {
    ...
    for (const table of ['pilot_exchanges','pilot_private_inputs','pilot_items','pilot_messages','pilot_setup_operations','pilot_approvals','pilot_operations','pilot_events','pilot_commands','pilot_assets','pilot_preferences','pilot_research','pilot_connections','pilot_service_actions']) {
      await assert.rejects(client.query(`select * from public.${table}`), /permission denied/);
    }
  } finally { await client.query('reset role'); client.release(); }
  assert.equal((await pool.query("select count(*)::int n from pg_class where relnamespace='public'::regnamespace and relname like 'pilot_%' and relkind='r' and relrowsecurity and relforcerowsecurity")).rows[0].n, 14);
